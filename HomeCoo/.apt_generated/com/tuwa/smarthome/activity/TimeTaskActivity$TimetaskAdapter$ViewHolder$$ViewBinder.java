// Generated code from Butter Knife. Do not modify!
package com.tuwa.smarthome.activity;

import android.view.View;
import butterknife.ButterKnife.Finder;
import butterknife.ButterKnife.ViewBinder;

public class TimeTaskActivity$TimetaskAdapter$ViewHolder$$ViewBinder<T extends com.tuwa.smarthome.activity.TimeTaskActivity.TimetaskAdapter.ViewHolder> implements ViewBinder<T> {
  @Override public void bind(final Finder finder, final T target, Object source) {
    View view;
    view = finder.findRequiredView(source, 2131231848, "field 'imBtnPause'");
    target.imBtnPause = finder.castView(view, 2131231848, "field 'imBtnPause'");
    view = finder.findRequiredView(source, 2131231841, "field 'tgBtn4'");
    target.tgBtn4 = finder.castView(view, 2131231841, "field 'tgBtn4'");
    view = finder.findRequiredView(source, 2131231849, "field 'imBtnOff'");
    target.imBtnOff = finder.castView(view, 2131231849, "field 'imBtnOff'");
    view = finder.findRequiredView(source, 2131231852, "field 'tvTime'");
    target.tvTime = finder.castView(view, 2131231852, "field 'tvTime'");
    view = finder.findRequiredView(source, 2131231842, "field 'tgBtn3'");
    target.tgBtn3 = finder.castView(view, 2131231842, "field 'tgBtn3'");
    view = finder.findRequiredView(source, 2131231844, "field 'tgBtn1'");
    target.tgBtn1 = finder.castView(view, 2131231844, "field 'tgBtn1'");
    view = finder.findRequiredView(source, 2131231847, "field 'imBtnOn'");
    target.imBtnOn = finder.castView(view, 2131231847, "field 'imBtnOn'");
    view = finder.findRequiredView(source, 2131231843, "field 'tgBtn2'");
    target.tgBtn2 = finder.castView(view, 2131231843, "field 'tgBtn2'");
    view = finder.findRequiredView(source, 2131231854, "field 'tvScheduleName'");
    target.tvScheduleName = finder.castView(view, 2131231854, "field 'tvScheduleName'");
    view = finder.findRequiredView(source, 2131231840, "field 'sbLight'");
    target.sbLight = finder.castView(view, 2131231840, "field 'sbLight'");
    view = finder.findRequiredView(source, 2131231855, "field 'tvWeek'");
    target.tvWeek = finder.castView(view, 2131231855, "field 'tvWeek'");
    view = finder.findRequiredView(source, 2131231856, "field 'imScheduleManager'");
    target.imScheduleManager = finder.castView(view, 2131231856, "field 'imScheduleManager'");
  }

  @Override public void unbind(T target) {
    target.imBtnPause = null;
    target.tgBtn4 = null;
    target.imBtnOff = null;
    target.tvTime = null;
    target.tgBtn3 = null;
    target.tgBtn1 = null;
    target.imBtnOn = null;
    target.tgBtn2 = null;
    target.tvScheduleName = null;
    target.sbLight = null;
    target.tvWeek = null;
    target.imScheduleManager = null;
  }
}
