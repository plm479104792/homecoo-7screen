...
public class ThemeDao {
	...
	public void deleteThemeByDeviceNo(String deviceno) {
		try {
			...
			deleteBuilder.delete();
			System.out.println("根据deviceno删除情景");
		} catch (SQLException e) {
			e.printStackTrace();
		}
	 ...
	 * @param theme
	 */
	public void updateThemeQuickByThemeNo(Theme theme) {
		String themeNo=theme.getThemeNo();
		String themeQuick = theme.getThemeQuick();
		try {
			UpdateBuilder<Theme, Integer> updateBuilder = themeDao
					.updateBuilder();
			updateBuilder.where().eq("THEME_NO", themeNo);
			updateBuilder.updateColumnValue("THEME_QUICK",themeQuick);
			updateBuilder.update();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
    ...
    * @return
    */
	public List<Theme> findThemeQuickListByGatewayNo(String gatewayNo) {
		List<Theme>  themeList=new ArrayList<Theme>();
		try {
			themeList= themeDao.queryBuilder().where()
					.eq("GATEWAY_NO", gatewayNo).and()
